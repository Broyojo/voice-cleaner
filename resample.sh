...
if [ $test == "0" ] #notest
then
     for i in $FILEPATHi*$extention ; do ffmpeg -i "$i" -acodec pcm_s32le -f sox -ac 1 -ar $br pipe: | sox -p -r 44100 -e signed-integer -b 16 -c 1 -t raw - | split -b 4410 -d --additional-suffix=.raw -a 10 - $FILEPATHo"${i%.*}" ; done 
fi
